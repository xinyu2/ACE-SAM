...
do
	for f0 in "${F0[@]}" ; 
	do
		time python test_cifar.py --config=./configs/Cifar10_2.json --work=train --clambda=$lam --f0=$f0
		time python test_cifar.py --config=./configs/Cifar10_2.json --work=test --clambda=$lam --f0=$f0
	done	
done
